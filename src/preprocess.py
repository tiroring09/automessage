...
ROOT_RESULT_DIR = join(ROOT_WORK_DIR, 'preprocess')
NATION = ['SG', 'MY', 'PH', 'ID']

###################### CONSTANTS ###################### END

for nation in NATION:
    WORK_DIR = join(ROOT_WORK_DIR, nation)
    RESULT_DIR = join(ROOT_RESULT_DIR, nation)
    ...
    for i, img in enumerate(img_list):
        ...
        order = cross_check_in_excel(ROOT_WORK_DIR, order)
        order['no'] = (i+1)
    ...
    # 로그파일 생성
    n = datetime.now()
    logger_name = f'{nation}_log_{n.month}{n.day}_{n.hour}{n.minute}.txt'
    ...
    with open(logger, 'w') as log:
        ...
        if len(orderid_list) != 0:
            log.write(f'total: ({len(orderid_list)}) / success: ({s_cnt}) / fail: ({f_cnt}) / success rate: ({s_cnt / len(orderid_list) * 100})')